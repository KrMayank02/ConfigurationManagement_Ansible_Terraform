# Declare variable for instance name

variable "instance_name" {
  type = list(string)
  default = ["web_instance_1","web_instance_2","web_instance_3","web_instance_4","web_instance_5"]
}
...
provider "aws" {
  access_key = "AKIAZOZQFIZZVHZ7OPGO"
...
resource "aws_instance" "web" {
  ...
  # Remote-exec Provisioner Block, waiting SSH connection
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i myhosts --private-key sl-key.pem tomcat-playbook.yaml"
 }
